#!/usr/bin/env python3
"""Stage and optionally package the @uxarion/aiplc npm module."""
...
CODEX_NPM_NAME = "@uxarion/aiplc"

# `npm_name` is the local optional-dependency alias consumed by `bin/codex.js`.
# The underlying package published to npm is always `@uxarion/aiplc`.
CODEX_PLATFORM_PACKAGES: dict[str, dict[str, str]] = {
    "aiplc-linux-x64": {
        "npm_name": "@uxarion/aiplc-linux-x64",
        "npm_tag": "linux-x64",
    ...
    "aiplc-linux-arm64": {
        "npm_name": "@uxarion/aiplc-linux-arm64",
        "npm_tag": "linux-arm64",
    ...
    "aiplc-darwin-x64": {
        "npm_name": "@uxarion/aiplc-darwin-x64",
        "npm_tag": "darwin-x64",
    ...
    "aiplc-darwin-arm64": {
        "npm_name": "@uxarion/aiplc-darwin-arm64",
    ...
    "aiplc-win32-x64": {
        "npm_name": "@uxarion/aiplc-win32-x64",
        "npm_tag": "win32-x64",
    ...
    "aiplc-win32-arm64": {
        "npm_name": "@uxarion/aiplc-win32-arm64",
        "npm_tag": "win32-arm64",
...
PACKAGE_EXPANSIONS: dict[str, list[str]] = {
    "aiplc": ["aiplc", *CODEX_PLATFORM_PACKAGES],
}

PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    "aiplc": [],
    "aiplc-linux-x64": ["codex", "rg"],
    "aiplc-linux-arm64": ["codex", "rg"],
    "aiplc-darwin-x64": ["codex", "rg"],
    "aiplc-darwin-arm64": ["codex", "rg"],
    "aiplc-win32-x64": ["codex", "rg", "codex-windows-sandbox-setup", "codex-command-runner"],
    "aiplc-win32-arm64": ["codex", "rg", "codex-windows-sandbox-setup", "codex-command-runner"],
    "aiplc-responses-api-proxy": ["codex-responses-api-proxy"],
    "aiplc-sdk": [],
}
...
COMPONENT_DEST_DIR: dict[str, str] = {
    ...
    "codex-windows-sandbox-setup": "aiplc",
    "codex-command-runner": "aiplc",
    "rg": "path",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build or stage the AIPLC npm package.")
    parser.add_argument(
        "--package",
        choices=PACKAGE_CHOICES,
        default="aiplc",
        help="Which npm package to stage (default: aiplc).",
    )
    parser.add_argument(
        "--version",
...
def main() -> int:
    ...
    try:
        ...
        if release_version:
            ...
            elif package == "aiplc-responses-api-proxy":
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="aiplc-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    ...
    if package == "aiplc":
        ...
        package_json["optionalDependencies"] = {
            ...
            for platform_package in PACKAGE_EXPANSIONS["aiplc"]
            if platform_package != "aiplc"
        }
...
def stage_codex_sdk_sources(staging_dir: Path) -> None:
    ...
    if not dist_src.exists():
        raise RuntimeError("aiplc-sdk build did not produce a dist directory.")
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="aiplc-npm-pack-") as pack_dir_str:
