...
#[clap(
    ...
    // `aiplc-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `aiplc` command name that users run.
    bin_name = "aiplc",
    override_usage = "aiplc [OPTIONS] [PROMPT]\n       aiplc [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
